...
def preprocessing(lines, dictionary):
    ...
    count_docs(stemmed_tokens, dictionary)  # construct the dictionary of words from each file
    ...
    for term in q:
        if term in positional_index.keys():
            for doc_id in positional_index[term][1].keys():
                if matches[doc_id]:
                    for i, val in enumerate(positional_index[term][1][doc_id]):
                        if matches[doc_id][-1] == positional_index[term][1][doc_id][i] - 1:
                            matches[doc_id].append(positional_index[term][1][doc_id][i])
                else:
...
def tf(terms_dictionary, document_type):
    ...
    if document_type == 'Document':
        ...
        t = Counter()
        for query_phrase in terms_dictionary:  # looping through incase there's multiple phrases
            t += (Counter(query_phrase))
        terms_dictionary = [t]
